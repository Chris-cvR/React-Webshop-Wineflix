...
async function addToCart(product_id) {
  ...
  if (product) {
    ...
    fetch(`http://127.0.0.1:8888/api/cart/${id}`, {
      method: 'POST',
      body: JSON.stringify({
        product_data: cart.product_data,
        count: count,
...
async function removeFromCart(product_id) {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:8888/api/cart/${user_id}`);
    if (response.ok) {
      ...
      if (productIndex !== -1) {
        ...
        const updateResponse = await fetch(`http://127.0.0.1:8888/api/cart/${user_id}`, {
          method: 'PUT',
